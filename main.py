...
parser.add_argument('--dataPath', default='', help='parent path of the folder of images')
...
parser.add_argument('--n_iter', type=int, default=8, help='number of epochs to train before changing the progress')
...
while True:
    ...
    for i, (images, _) in enumerate(data_loader):
        ...
        if opt.CM:
            do_cm = torch.rand(1)
            if do_cm > 0.75:
                fake_image_for_cm = fake_images.clone()
                mask, how_many_real = generate_random_mask(fake_image_for_cm.shape[2])
                mask = torch.from_numpy(mask).to(DEVICE).unsqueeze(0).type(torch.float)

                fake_real_cm = mask * images + (1-mask) * fake_image_for_cm

                D_fake_cm = D(fake_real_cm, P.p)
                D_fakem = D_fake_cm.mean()
            else:
                D_fakem = D(fake_images, P.p).mean()
        ...
        if opt.CM:
            if do_cm > 0.75:
                d_loss = D_fakem * (1 - 1/how_many_real) - D_realm
            else:
